...
void TuioServer::sendFullMessages() {
	...
	for (std::list<TuioCursor*>::iterator tuioCursor = cursorList.begin(); tuioCursor!=cursorList.end(); tuioCursor++)
		(*fullPacket) << (int64)((*tuioCursor)->getSessionID());
	...
	for (std::list<TuioCursor*>::iterator tuioCursor = cursorList.begin(); tuioCursor!=cursorList.end(); tuioCursor++) {
		...
		if ((fullPacket->Capacity()-fullPacket->Size())<CUR_MESSAGE_SIZE) {
			...
			(*fullPacket) << osc::BeginMessage( "/tuio/2Dcur") << "fseq" << (int64)-1 << osc::EndMessage;
		...
		(*fullPacket) << (int64)((*tuioCursor)->getSessionID()) << (*tuioCursor)->getX() << (*tuioCursor)->getY();
	...
	for (std::list<TuioObject*>::iterator tuioObject = objectList.begin(); tuioObject!=objectList.end(); tuioObject++)
		(*fullPacket) << (int64)((*tuioObject)->getSessionID());
	(*fullPacket) << osc::EndMessage;	
	...
	for (std::list<TuioObject*>::iterator tuioObject = objectList.begin(); tuioObject!=objectList.end(); tuioObject++) {
		...
		if ((fullPacket->Capacity()-fullPacket->Size())<OBJ_MESSAGE_SIZE) {
			...
			(*fullPacket) << osc::BeginMessage( "/tuio/2Dobj") << "fseq" << (int64)-1 << osc::EndMessage;
			(*fullPacket) << osc::EndBundle;
			socket->Send( fullPacket->Data(), fullPacket->Size() );
		...
		(*fullPacket) << (int64)((*tuioObject)->getSessionID()) << (int64)(*tuioObject)->getSymbolID() << (int64)(*tuioObject)->getX() << (int64)(*tuioObject)->getY() << (int64)(*tuioObject)->getAngle();
...
void TuioServer::sendEmptyCursorBundle() {
	...
	(*oscPacket) << osc::BeginMessage( "/tuio/2Dcur") << "fseq" << (int64)-1 << osc::EndMessage;
...
void TuioServer::startCursorBundle() {	
	...
	for (std::list<TuioCursor*>::iterator tuioCursor = cursorList.begin(); tuioCursor!=cursorList.end(); tuioCursor++) {
		(*oscPacket) << (int64)((*tuioCursor)->getSessionID());
	}
...
void TuioServer::addCursorMessage(TuioCursor *tcur) {
	 ...
	 (*oscPacket) << (int64)(tcur->getSessionID()) << tcur->getX() << tcur->getY();
	 (*oscPacket) << tcur->getXSpeed() << tcur->getYSpeed() << tcur->getMotionAccel();	
	 (*oscPacket) << osc::EndMessage;
}

void TuioServer::sendCursorBundle(long fseq) {
	(*oscPacket) << osc::BeginMessage( "/tuio/2Dcur") << "fseq" << (int64)fseq << osc::EndMessage;
	(*oscPacket) << osc::EndBundle;
	socket->Send( oscPacket->Data(), oscPacket->Size() );
}
...
void TuioServer::sendEmptyObjectBundle() {
	...
	(*oscPacket) << osc::BeginMessage( "/tuio/2Dobj") << "fseq" << (int64)-1 << osc::EndMessage;
	(*oscPacket) << osc::EndBundle;
	socket->Send( oscPacket->Data(), oscPacket->Size() );
}
...
void TuioServer::startObjectBundle() {
	...
	for (std::list<TuioObject*>::iterator tuioObject = objectList.begin(); tuioObject!=objectList.end(); tuioObject++) {
		(*oscPacket) << (int64)((*tuioObject)->getSessionID());
	}
	(*oscPacket) << osc::EndMessage;	
}
...
void TuioServer::addObjectMessage(TuioObject *tobj) {
	...
	(*oscPacket) << (int64)(tobj->getSessionID()) << (int64)tobj->getSymbolID() << (int64)tobj->getX() << (int64)tobj->getY() << (int64)tobj->getAngle();
...
void TuioServer::sendObjectBundle(long fseq) {
	(*oscPacket) << osc::BeginMessage( "/tuio/2Dobj") << "fseq" << (int64)fseq << osc::EndMessage;
	(*oscPacket) << osc::EndBundle;
	socket->Send( oscPacket->Data(), oscPacket->Size() );
}
